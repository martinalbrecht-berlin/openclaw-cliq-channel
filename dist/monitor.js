...
function verifyWebhookSecret(req, secret) {
    ...
    const provided = req.headers["x-cliq-webhook-secret"] ??
        req.headers["x-webhook-secret"] ??
        req.headers["authorization"]?.replace(/^Bearer\s+/i, "");
    return !!provided && provided === secret;
}
// ─── Parse Cliq Payload ──────────────────────────────────────────────────────
function parseCliqPayload(body) {
    ...
    // Detect mention
    const isMention = body.handler === "mention" ||
...
export async function handleCliqWebhookRequest(req, res, cfg) {
    ...
    try {
        ...
        console.log(`[cliq] Inbound: ${parsed.chatType} ${parsed.chatId} from ${parsed.senderName}: ${parsed.text.slice(0, 80)}`);
