...
import { c as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-D0l5Auhm.js";
import { a as stripPendingPluginInstallRecords, i as hasPendingPluginInstallRecords, n as commitConfigWriteWithPendingPluginInstalls, s as unchangedPendingPluginInstallRecordIds } from "./plugins-install-record-commit-BoluiVmq.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-CsXcEckS.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-BihmuuQH.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-D7DdmW9N.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-5uQyBFGH.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-HPi0UF09.js");
	return modelPickerModulePromise;
}
function loadOnboardConfigModule() {
	onboardConfigModulePromise ??= import("./onboard-config-CYcR0z_o.js");
...
async function runSetupWizard(opts, runtimeInput, prompter) {
	...
	const onboardHelpers = await import("./onboard-helpers-DqWcKPW2.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-BTS4vjMW.js");
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-yunGWfLy.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-B34ufFGk.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note(t("wizard.setup.skipChannels"), t("wizard.setup.channelsTitle"));
		...
		const { setupChannels } = await import("./onboard-channels-C-QLqD_C.js");
	...
	if (opts.skipSearch) await prompter.note(t("wizard.setup.skipSearch"), t("wizard.setup.searchTitle"));
		...
		const { setupSearch } = await import("./onboard-search-DdV23l8Y.js");
	...
	if (opts.skipSkills) await prompter.note(t("wizard.setup.skipSkills"), t("wizard.setup.skillsTitle"));
		...
		const { setupSkills } = await import("./onboard-skills-DsTjwt8m.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-DvsaHyB4.js");
	...
	nextConfig = await writeSetupConfigFile(nextConfig, { allowConfigSizeDrop: configResetPerformed });
	const { finalizeSetupWizard } = await import("./setup.finalize-DLxLAAPo.js");
