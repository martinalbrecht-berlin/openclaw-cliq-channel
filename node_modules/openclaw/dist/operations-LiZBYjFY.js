...
const loadOverviewModule = async () => await import("./overview-C9j5CKmV.js");
const loadModelsSharedModule = async () => await import("./shared-CydOj6MO.js");
const loadConfigCliModule = async () => await import("./config-cli-DEB9ECqi.js");
const loadDoctorModule = async () => await import("./doctor-BgkvFhfl.js");
...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-KBARno2u.js");
	if (operation === "start") {
...
async function executeCrestodianOperation(operation, runtime, opts = {}) {
	...
	if (operation.kind === "plugin-install") {
		...
		await (opts.deps?.runPluginInstall ?? (async (spec, pluginRuntime) => {
			const { runPluginInstallCommand } = await import("./plugins-install-command-DVZacBfP.js");
	...
	if (operation.kind === "plugin-uninstall") {
		...
		await (opts.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
			const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-5Oq4LLjN.js");
	...
	if (operation.kind === "create-agent") {
		...
		const before = await readConfigFileSnapshot();
		const workspace = resolveUserPath(operation.workspace ?? process.cwd());
		await (opts.deps?.runAgentsAdd ?? (await import("./agents.commands.add-C6XxAp1N.js")).agentsAddCommand)({
	...
	if (operation.kind === "status") {
		...
		const { statusCommand } = await import("./status.command-BAUZFBHL.js");
	...
	if (operation.kind === "open-tui") {
		...
		const result = await (opts.deps?.runTui ?? (await import("./tui-DS7tQ8_I.js")).runTui)({
			local: true,
			session,
			deliver: false,
			historyLimit: 200
