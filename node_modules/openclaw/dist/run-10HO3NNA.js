...
async function runGatewayCommand(opts, hooks = {}) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-B3yIJBuK.js")));
	...
	if (devMode) {
		...
		const { ensureDevGatewayConfig } = await import("./dev-CEa7QOff.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			const { maybeExplainGatewayServiceStop } = await import("./shared-DTRdY6Nv.js");
